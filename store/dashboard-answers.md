...
> No login required. Open
> https://www.immotop.lu/search-list/?idContratto=2&idCategoria=1&criterio=rilevanza&__lang=en
> and wait for listings. Hover an agency logo (top-right of a card, e.g. Solcolux) to reveal a
> round button; click it to blur that seller's listings with an overlay and Undo toast. Click
> Show anyway to reveal one. Open the toolbar popup to see/remove blocked sellers, test
> export/import, and switch language.
